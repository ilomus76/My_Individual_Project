...
korean_text = f"국어 평균:{sum(korean_score)/(len(each_data_line)-1)} 최고점:{max(korean_score)} 최저점:{min(korean_score)}"
english_text = f"영어 평균:{sum(english_score)/(len(each_data_line)-1)} 최고점:{max(english_score)} 최저점:{min(english_score)}"
math_text = f"수학 평균:{sum(math_score)/(len(each_data_line)-1)} 최고점:{max(math_score)} 최저점:{min(math_score)}"
