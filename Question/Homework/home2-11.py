...
class StudentScore:
    ...
    def __init__(self, list_of_list): # 객체가 생성되면서 바로 시작되는 초기화(객체생성자)함수
        self.score_data = list_of_list
        self.score_info_dict_by_subject = {}
        print(len(self.score_data))
        self.n=0

        
    def data_sum_by_subject(self):
        # print(self.score_data)        
        for self.n in range(1,len(self.score_data)):
            print(self.score_data[0][3:5]) +=0 #+= 0# self.score_data[0][1]
            #print(self.score_info_dict_by_subject[self.score_data[0][1]]) #+=0 #+= 0# self.score_data[0][1]
            # score_info_dict_by_subject[self.socre_data[0][2]] +=  self.socre_data[i][2]
            # score_info_dict_by_subject[self.socre_data[0][2]] +=  self.socre_data[i][3]
            

        # total = self.subject_a + self.subject_b + self.subject_c
        return #self.score_info_dict_by_subject
...
with open('Question/Homework/scores.csv','r',encoding="UTF-8") as file:
    ...
    # 객체에게 정제된 데이타를 송부 
    indicator_score = StudentScore(data)
    sum_score_dict = indicator_score.data_sum_by_subject()
