...
#5. 모든셀의 글씨를 가운데 정렬 및 80점이상인 셀들은 노란색으로 표식!
...
for row in sheet.rows:
    for cell in row:
        ...
        if cell.column == 1 : # 첫번째 칸(번호 칸)은 점수가 아니니 계산하지 않도록
            continue

        #번호 제외하고.. 국어,영어 점수들 중 80점 이상인 셀의 배격색을 노랑색으로..
        if isinstance(cell.value, int) and cell.value>=80 :
            cell.fill= PatternFill(fgColor='FFFF00', fill_type='solid')
            cell.font= Font(color='FF0000')

# 데이터가 많아지면 스클롤 됨. 이때 특정 위치를 고정시키기 [틀 고정]
